...
def semicriterion(inputs, targets):
    ...
    return bce
def uncriterion(inputs, targets):
    # print(inputs.shape, targets.shape)
    # bce = weight_binary_cross_entropy_loss(inputs,targets)
    ...
    return loss
def supcriterion(inputs, targets):
    # print(inputs.shape, targets.shape)
    # bce = weight_binary_cross_entropy_loss(inputs,targets)
...
def get_arguments():
    ...
    parser.add_argument('--ratio', '--labeled_ratio', type=float, default=0.1) # labeled data ratio
    parser.add_argument('--mask_size', type=list, default=[64, 64])
    parser.add_argument("--log", type=str, default='cutmix_0.1')
    parser.add_argument("--data_root", type=str, default='/home/data3/liangyizhou/Data/levir')
...
def train():
    ...
    NCloss = InfoNCE(negative_mode='unpaired')
    PCloss = InfoNCE()
    ...
    for epoch_i in range(args.num_epochs):
        ...
        for sup_batch in tqdm(itertools.islice(train_sup_iter, iters)):  # 一个epoch包含的iteration数
            ...
            logits_sup,x_1,x_2 = student_net(batch_x)
            # Mseloss = mseloss(x_1,batch_x[0])+mseloss(x_2,batch_x[1])
            ...
            if args.cons_weight > 0.0 and epoch_i >= args.epoch_start_unsup:
                ...
                with torch.no_grad():
                    logits_u0_tea,Ta0,Tb0 = teacher_net(batch_ux0_tea)
                    logits_u0_tea,Ta0,Tb0 =  logits_u0_tea.detach(),Ta0.detach(),Tb0.detach()
                    logits_u1_tea,Ta1,Tb1 = teacher_net(batch_ux1_tea)
                    logits_u1_tea,Ta1,Tb1 = logits_u1_tea.detach(),Ta1.detach(),Tb1.detach()

                # generate salience mask from logits_u1_tea  [B, 2, H, W]
                # change-aware masks
                batch_mix_masks = generate_salience_mask(logits_u1_tea).to(torch_device)    # [B,1,H,W] 中间的mask为0
                batch_mix_masks0 = generate_salience_mask(logits_u0_tea).to(torch_device)
                ...
                logits_cons_stu, feat_mixed,ux_1,ux_2 = student_net(batch_ux_stu_mixed, return_features=True)   # output the Feature contrastive loss
                Nux_10,Nux_20 = student_net(batch_ux_stu_mixed,mask=batch_mix_masks0)
                Nux_11,Nux_21 = student_net(batch_ux_stu_mixed,mask=batch_mix_masks )
                Pux_10,Pux_20 = student_net(batch_ux_stu_mixed,mask=1-batch_mix_masks0)
                Pux_11,Pux_21 = student_net(batch_ux_stu_mixed,mask=1-batch_mix_masks )
                ...
                feat_contras_loss = NCloss(Nux_10,Nux_10,Nux_20)+NCloss(Nux_11,Nux_11,Nux_21)

                # unsupervised loss and back-prop
                # uMseloss = mseloss(ux_1,batch_ux_stu_mixed[0]) + mseloss(ux_2,batch_ux_stu_mixed[1])
                unsup_loss = consistency_loss * args.cons_weight + feat_contras_loss.mean()
               # unsup_loss = unsup_loss + contraloss
                unsup_loss.backward()
